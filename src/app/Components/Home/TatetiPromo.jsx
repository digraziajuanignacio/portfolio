...
export default function TatetiPromo() {
  return (
    <section className="tateti-promo-section py-5">
      ...
      <style jsx>{`
        .promo-card { background: var(--card-bg); border-radius: 30px; padding: 3rem; max-width: 1000px; box-shadow: 0 20px 60px rgba(0,0,0,0.06); border: 1px solid var(--border-color); position: relative; overflow: hidden; }
        .promo-card::after { content: ""; position: absolute; top: -50px; right: -50px; width: 200px; height: 200px; background: radial-gradient(circle, rgba(32, 201, 151, 0.1) 0%, transparent 70%); border-radius: 50%; }
        ...
        .game-preview-box { background: rgba(0,0,0,0.02); padding: 20px; border-radius: 20px; display: inline-block; border: 1px dashed var(--border-color); }
        [data-theme='dark'] .game-preview-box { background: rgba(255,255,255,0.02); }
        .preview-grid { display: grid; grid-template-columns: repeat(3, 40px); gap: 8px; }
        .preview-sq { width: 40px; height: 40px; background: var(--bg-light); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 1.2rem; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
        .preview-sq.X { color: var(--secondary-color); }
        .preview-sq.O { color: var(--primary-color); }
      `}</style>
    </section>
  );
}
